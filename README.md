...
| **启用压缩文件预览** (`enable_zip_preview`)                        | bool   | `true`  | 开启后，插件会尝试解压收到的压缩包（支持 ZIP/7Z/TAR/GZ/BZ2/XZ 等格式），并对其中的文本文件进行内容预览。                                                                                     |
| **压缩文件解压大小上限 (MB)** (`zip_extraction_size_limit_mb`)         | int    | `100`   | 压缩文件大小**超过此值**时，将**不会**尝试解压以获取预览。设置为 `0` 则表示不限制。                                                           |
| **压缩文件默认解压密码** (`default_zip_password`)                  | string | `""`    | 当直接解压失败时，会尝试使用此密码进行解压（支持 ZIP/7Z 等加密格式）。留空表示不使用密码重试。                                                                                    |
