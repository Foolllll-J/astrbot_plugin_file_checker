...
class PreviewManager:
    ...
    async def _convert_file_to_media(
        self,
        ...
        if local_path is None or not os.path.exists(local_path):
            logger.warning(
                f"[{group_id}] ⚠️ 无可用本地文件，跳过{media_type}转换: {file_name}"
            )
            return
        ...
        try:
            ...
            logger.debug(
                f"[{group_id}] 📤 准备以{media_name}形式发送文件 ({file_size_mb:.2f} MB): {absolute_path}"
            )
            ...
            logger.info(
                f"[{group_id}] ✅ {media_name}发送成功，将在 30 分钟后删除群文件"
            )

            # 30分钟后删除群文件（本地文件由 main.py 统一清理）
            delete_delay = 1800  # 30分钟
            asyncio.create_task(
                self._delayed_cleanup_group_file_only(event, file_name, delete_delay)
            )
    ...
    async def _delayed_cleanup_group_file_only(
        self, event: AstrMessageEvent, file_name: str, delay: int
    ):
    ...
    async def _get_preview_from_archive(
        self, file_path: str, file_name: str, preview_config: dict = None
    ) -> tuple[str, str]:
        """通用压缩包预览方法，支持多种格式"""
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *command_no_pwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            if process.returncode != 0:
                ...
                logger.error(
                    f"无密码解压 {archive_type} 失败 (返回码 {process.returncode}): {error_message}"
                )
                if default_zip_password:
                    logger.info(
                        f"无密码解压 {archive_type} 失败，正在尝试使用默认密码..."
                    )
                    command_with_pwd = [
                        "7za",
                        "x",
                    ...
                    if process.returncode != 0:
                        ...
                        logger.error(
                            f"使用默认密码解压 {archive_type} 失败: {error_message}"
                        )
                        return "", f"{archive_type} 解压失败"
                else:
                    error_message = stderr.decode("utf-8")
                    logger.error(
                        f"使用 7za 命令解压 {archive_type} 失败且未设置默认密码: {error_message}"
                    )
                    return "", f"{archive_type} 解压失败"
            ...
            if len(previewable_files) == 1 and len(all_extracted_files) == 1:
                ...
                if re.search(r'[\\/|*<>;"\x00-\x1F\x7F]', safe_text_name):
                    logger.error(
                        f"解压出的文件名 '{safe_text_name}' 包含非安全字符，跳过预览。"
                    )
                    return "", "解压出的文件名不安全"
        ...
        try:
            with zipfile.ZipFile(epub_path, "r") as z:
                ...
                for item_path in spine_items:
                    ...
                    try:
                        html_content = z.read(item_path).decode(
                            "utf-8", errors="ignore"
                        )
    ...
    def _should_download_for_preview(
        self, file_name: str, file_size: Optional[int], preview_config: dict = None
    ) -> bool:
        """判断是否需要为预览下载文件"""
        if preview_config is None:
            preview_config = {}
        preview_length = preview_config.get("preview_length", 500)
        zip_extraction_size_limit_mb = preview_config.get(
            "zip_extraction_size_limit_mb", 0
        )
        ...
        is_archive = zip_extraction_size_limit_mb > 0 and self._is_archive_file(
            file_name
        )
    ...
    async def _get_preview_for_file(
        self,
        file_name: str,
        local_path: Optional[str],
        ...
        if is_archive:
            preview_text, extra_info = await self._get_preview_from_archive(
                local_path, file_name, preview_config
            )
            return preview_text, extra_info
    ...
    async def _get_pdf_preview(
        self, file_path: str, preview_config: dict = None
    ) -> List[str]:
    ...
    def send_pdf_preview(
        self, event: AstrMessageEvent, message_text: str, pdf_preview_images: List[str]
    ):
        """使用合并转发发送 PDF 预览图"""
        ...
        pdf_preview_nodes = [
            Comp.Node(
                uin=sender_id, name="PDF 预览", content=[Comp.Plain(message_text)]
            )
        ]
        for img in pdf_preview_images:
            pdf_preview_nodes.append(
                Comp.Node(
                    uin=sender_id,
                    name="PDF 预览",
                    content=[Comp.Image.fromFileSystem(img)],
                )
            )
